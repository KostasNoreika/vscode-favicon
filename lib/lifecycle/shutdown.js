 ...
 * 3. Save pending notifications to disk
 * 4. Close registry file watcher
 ...
 */
async function gracefulShutdown(signal, server, cleanupInterval) {
    ...
    try {
        ...
        // Step 2: Stop cleanup interval (synchronous, safe to run independently)
 ...
 * @param {NodeJS.Timeout} cleanupInterval - Notification cleanup interval ID
 */
function registerShutdownHandlers(server, cleanupInterval) {
    // Register signal handlers for graceful shutdown
    process.on('SIGTERM', () => gracefulShutdown('SIGTERM', server, cleanupInterval));
    process.on('SIGINT', () => gracefulShutdown('SIGINT', server, cleanupInterval));

    // Handle uncaught errors
    process.on('uncaughtException', (err) => {
        logger.fatal({ err }, 'Uncaught exception');
        gracefulShutdown('uncaughtException', server, cleanupInterval);
    });
