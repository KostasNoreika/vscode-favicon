...
function corsMiddleware(req, res, next) {
    ...
    // Note: Chrome extensions send Origin: chrome-extension://<extension-id>
    // We allow extension origins so the bundled browser extension can call the API.
    const isAllowedExtensionOrigin =
        typeof origin === 'string' && (origin.startsWith('chrome-extension://') || origin.startsWith('moz-extension://'));

    if (origin && (ALLOWED_ORIGINS.includes(origin) || isAllowedExtensionOrigin)) {
        // Set CORS headers ONLY for whitelisted origins (plus browser extension origins)
