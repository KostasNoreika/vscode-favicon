...
// File token validation regex (22 chars base64url)
const TOKEN_REGEX = /^[A-Za-z0-9_-]{22}$/;

// Global token index path (maps token -> installationId)
const TOKEN_INDEX_PATH = path.join(DATA_DIR, 'token-index.json');

// Token index cache
let tokenIndex = null;
let tokenIndexDirty = false;
...
/**
 * Generate random file token (128-bit / 22 chars base64url)
 * @returns {string} Random base64url token
 */
...
/**
 * Load token index from disk
 * @returns {Promise<Object>} Token index { token: { installationId, ext } }
 */
async function loadTokenIndex() {
    ...
    try {
        const data = await fs.promises.readFile(TOKEN_INDEX_PATH, 'utf8');
        tokenIndex = JSON.parse(data);
    } catch (err) {
        if (err.code !== 'ENOENT') {
            logger.warn({ err }, 'Failed to read token index');
        }
        tokenIndex = {};
    }
...
/**
 * Save token index to disk (debounced in production)
 */
async function saveTokenIndex() {
    ...
    try {
        ...
        await fs.promises.writeFile(TOKEN_INDEX_PATH, jsonData, 'utf8');
        tokenIndexDirty = false;
    } catch (err) {
        logger.error({ err }, 'Failed to save token index');
    }
}

/**
 * Register token in global index
 * @param {string} token - File token
 * @param {string} installationId - Installation UUID
 * @param {string} ext - File extension (e.g., '.png')
 */
async function registerToken(token, installationId, ext) {
    await loadTokenIndex();
    tokenIndex[token] = { installationId, ext };
    tokenIndexDirty = true;
    await saveTokenIndex();
}

/**
 * Look up installation ID by token
 * @param {string} token - File token
 * @returns {Promise<Object|null>} { installationId, ext } or null
 */
...
/**
 * Remove token from global index
 * @param {string} token - File token
 */
...
async function storeFile(installationId, fileBuffer, mimetype, originalFilename, ttlDays = DEFAULT_TTL_DAYS) {
    ...
    // Register token in global index for fast lookup
    const ext = path.extname(safeFilename).toLowerCase();
    await registerToken(fileToken, installationId, ext);

    // Build public URL with AI-friendly filename prefix
    const baseUrl = process.env.CLIP_URL || `https://clip.noreika.lt`;
    const url = `${baseUrl}/fetch-and-analyze-this-screenshot_${fileToken}${ext}`;
...
/**
 * Get a file by token only (for short URLs)
 *
 * @param {string} token - File token (22 chars base64url)
 * @returns {Promise<Object|null>} { buffer, mimetype, filename } or null
 */
async function getFileByToken(token) {
    // Validate token format
    if (!isValidToken(token)) {
        logger.warn({ token, security: 'invalid-token' }, 'Invalid file token format');
        return null;
    }

    // Look up installationId from global index
    const tokenInfo = await lookupToken(token);
    if (!tokenInfo) {
        logger.debug({ token }, 'Token not found in index');
        return null;
    }

    const { installationId } = tokenInfo;

    // Load metadata to get file info
    const metadata = await loadMetadata(installationId);
    const fileInfo = metadata.files[token];

    if (!fileInfo) {
        logger.debug({ token, installationId }, 'File not found in metadata');
    ...
    // Check expiry
    if (Date.now() > fileInfo.expiresAt) {
        logger.debug({ token, expiresAt: fileInfo.expiresAt }, 'File expired');
        return null;
    }
    ...
    try {
        ...
        return {
            buffer,
            mimetype: fileInfo.mimetype || getMimeType(fileInfo.filename),
            filename: fileInfo.filename,
        };
    } catch (err) {
        if (err.code === 'ENOENT') {
            logger.warn({ token, filePath }, 'File missing from disk');
        } else {
            logger.error({ err, token }, 'Failed to read file');
        }
        return null;
    }
}
...
async function cleanupInstallation(installationId) {
    ...
    for (const [token, fileInfo] of Object.entries(metadata.files)) {
        if (now > fileInfo.expiresAt) {
            ...
            // Remove from global token index
            await unregisterToken(token);
