...
const { FileNotFoundError, PermissionError } = require('../errors');
...
class FaviconService {
    ...
    /**
     * Static method for generating cache keys (FIX REF-026)
     * Exposes centralized cache key generation for testing and external use
     *
     * @param {string} type - Key type (e.g., 'color', 'favicon', 'registry')
     * @param {...(string|number|boolean)} parts - Key parts to join
     * @returns {string} Versioned cache key
     * @throws {Error} If type is invalid
     * @throws {TypeError} If parts contain non-primitive values
     */
    static makeCacheKey(type, ...parts) {
        return makeCacheKey(type, ...parts);
    }
