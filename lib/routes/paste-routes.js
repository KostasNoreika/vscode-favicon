 ...
 * These errors typically indicate temporary resource unavailability
 */
 ...
 * Implements exponential backoff for retryable error codes
 *
...
async function retryTransientErrors(operation, operationName, options = RETRY_CONFIG) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            // If not retryable or out of retries, fail fast
            if (!isRetryable || attempt === maxRetries) {
                if (isRetryable && attempt === maxRetries) {
                    logger.warn(
                        ...
                        'Max retries exhausted for transient filesystem error'
                    );
                }
                throw error;
            }
            ...
            const delayMs = (options.initialDelayMs || RETRY_CONFIG.initialDelayMs) *
                           Math.pow(options.backoffMultiplier || RETRY_CONFIG.backoffMultiplier, attempt);
            ...
            logger.debug(
                ...
                'Retrying transient filesystem error'
            );
