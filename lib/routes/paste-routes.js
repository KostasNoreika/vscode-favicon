...
const { ALLOWED_MIME_TYPES, validateFileContent, generateTimestampedFilename } = require('../services/file-validator');
 ...
 * REF CENT-001: Uses centralized storage instead of project-local storage
 *
 * @param {Function} rateLimiter - Rate limiting middleware (100 req/min)
 * @returns {Object} Express router with paste routes
 */
function createPasteRoutes(rateLimiter) {
    /**
     ...
     * - Body fields:
     *   - folder: Project folder path (for logging/context)
     *   - image: Image file (png/jpeg/webp, max 10MB)
     *   - installationId: Extension installation UUID (required)
     *   - ttlDays: File retention period in days (optional, default 7)
     *
     * Response:
     * - 200: { success: true, filename: "...", url: "https://...", expiresAt: 1234567890 }
    ...
    router.post(
        ...
        // Note: requireValidPath removed - we use centralized storage now
        async (req, res) => {
            try {
                ...
                // Validate installation ID (required for centralized storage)
                const installationId = req.body.installationId;
                if (!installationId || !uploadStorage.isValidUUID(installationId)) {
                    req.log.warn({ installationId }, 'Invalid or missing installation ID');
                    return sendError(res, 400, ErrorCodes.INVALID_PARAMETER, 'Please update extension');
                }

                // Parse TTL (optional, default 7 days)
                const ttlDays = parseInt(req.body.ttlDays, 10) || 7;
                ...
                // Store file in centralized storage
                const uploadResult = await uploadStorage.storeFile(
                    installationId,
                    req.file.buffer,
                    validatedMimeType,
                    filename,
                ...
                req.log.info(
                    ...
                    'File uploaded to centralized storage'
                );
                ...
                if (error.message.includes('Maximum file count')) {
                    return sendError(res, 429, ErrorCodes.RATE_LIMITED, 'Too many files uploaded');
                }
                if (error.message.includes('Maximum storage size')) {
                    return sendError(res, 413, ErrorCodes.FILE_TOO_LARGE, 'Storage limit exceeded');
                }
                if (error.message.includes('Invalid installation ID')) {
                    return sendError(res, 400, ErrorCodes.INVALID_PARAMETER, 'Invalid installation ID');
                }
