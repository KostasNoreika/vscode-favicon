...
function createNotificationRoutes(requireValidPath, notificationLimiter) {
    ...
    router.post(
        ...
        requireValidPath,
        async (req, res) => {
            const { message = 'Task completed', metadata = null, origin = null } = req.body;
            const { validatedPath } = req;

            notificationStore.setCompleted(validatedPath, message, metadata, origin);

            req.log.info({ folder: validatedPath, message, hasMetadata: !!metadata, origin }, 'Claude completion notification stored');
    ...
    router.post(
        ...
        requireValidPath,
        async (req, res) => {
            const { message = 'Working...', origin = null } = req.body;
            const { validatedPath } = req;

            notificationStore.setWorking(validatedPath, message, origin);

            req.log.info({ folder: validatedPath, message, origin }, 'Claude started notification stored');
    ...
    router.get('/api/notifications/unread', notificationLimiter, async (req, res) => {
        try {
            ...
            const notifications = unreadNotifications.map(notification => {
                ...
                const fullNotification = notificationStore.get(notification.folder, origin);
    ...
    router.get(
        ...
        async (req, res) => {
            const { origin = null } = req.query;
            const { validatedPath } = req;
            const notification = notificationStore.get(validatedPath, origin);
    ...
    router.post(
        ...
        requireValidPath,
        async (req, res) => {
            const { origin = null } = req.body;
            const { validatedPath } = req;

            if (notificationStore.markRead(validatedPath, origin)) {
                req.log.info({ folder: validatedPath, origin }, 'Notification marked as read');
            }
    ...
    router.delete(
        ...
        async (req, res) => {
            ...
            if (notificationStore.remove(validatedPath, origin)) {
                req.log.info({ folder: validatedPath, origin }, 'Notification cleared');
            }
            res.json({ status: 'ok' });
        }
    );
