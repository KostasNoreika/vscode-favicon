...
// PUBLIC_MODE: When ALLOWED_PATHS contains '*', allow any valid absolute path
// This is useful for public APIs where path whitelisting is not needed
// Security is still maintained via sanitizePath() which blocks traversal attacks
const PUBLIC_MODE = ALLOWED_PATHS.length === 1 && ALLOWED_PATHS[0] === '*';
...
// Skip regex compilation in public mode (not needed)
const PATH_REGEXES = PUBLIC_MODE ? [] : ALLOWED_PATHS.map((allowedPath) => {
...
const ALLOWED_PREFIXES = PUBLIC_MODE ? [] : ALLOWED_PATHS.map((p) => p + '/');
const ALLOWED_EXACT_MATCHES = PUBLIC_MODE ? new Set() : new Set(ALLOWED_PATHS);
...
async function validatePathCore(sanitized) {
    // PUBLIC_MODE: Skip whitelist check, allow any valid absolute path
    // Security is maintained via sanitizePath() which blocks traversal attacks
    if (!PUBLIC_MODE) {
    ...
    // In PUBLIC_MODE, skip this check - any valid absolute path is allowed
    const isAllowed = PUBLIC_MODE ? true : checkPathAgainstAllowed(normalizedPath);
