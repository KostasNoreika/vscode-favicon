...
function set(folder, data) {
    ...
    index.updateUnreadIndex(folder, notifications);
    ...
    events.emit(folder, 'created', notifications[folder]);
}
...
function setWorking(folder, message = 'Working...') {
    ...
    events.emit(folder, 'working', notifications[folder]);
}
...
function setCompleted(folder, message = 'Task completed', metadata = null) {
    ...
    events.emit(folder, 'completed', notifications[folder]);
}
...
function markRead(folder) {
    if (notifications[folder]) {
        ...
        // Emit event for SSE clients
        events.emit(folder, 'read', notifications[folder]);
