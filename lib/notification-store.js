...
/**
 * Pure function: Sort notifications by timestamp (oldest first)
 * @param {Array<[string, Object]>} entries - Array of [folder, data] entries
 * @returns {Array<[string, Object]>} Sorted array (oldest first)
 */
function sortByTimestamp(entries) {
    return entries.slice().sort((a, b) => a[1].timestamp - b[1].timestamp);
}

/**
 * Pure function: Select N oldest notifications
 * @param {Array<[string, Object]>} entries - Array of [folder, data] entries
 * @param {number} n - Number of oldest entries to select
 * @returns {Array<[string, Object]>} N oldest entries
 */
function selectOldestN(entries, n) {
    if (n <= 0) return [];
    if (n >= entries.length) return sortByTimestamp(entries);
    return sortByTimestamp(entries).slice(0, n);
}
...
/**
 * PERF-001: Cleanup expired and excess notifications with race condition protection
 * - Removes notifications older than TTL_MS (24 hours)
 * - Enforces MAX_NOTIFICATIONS limit (keeps most recent)
 * - Uses mutex to prevent concurrent cleanup corruption
...
async function cleanup() {
    ...
    try {
        ...
        // Enforce size limit - simplified algorithm (always sort)
        ...
        if (remaining.length > MAX_NOTIFICATIONS) {
            ...
            // Use sort approach for all cases (negligible perf diff for <10000 items)
            const oldest = selectOldestN(remaining, excess);
        ...
        // PERF-003: Update minTimestamp cache after cleanup
        if (before > Object.keys(notifications).length) {
            index.updateMinTimestamp(notifications);
        }
...
function enforceSizeLimit() {
    ...
    if (remaining.length > MAX_NOTIFICATIONS) {
        ...
        if (oldest) {
            ...
            // PERF-003: Update minTimestamp cache after removal
            index.updateMinTimestamp(notifications);
            logger.debug(
                {
                    removed: oldest,
...
function set(folder, data, origin = null) {
    ...
    // PERF-003: Update minTimestamp cache after adding notification
    index.updateMinTimestamp(notifications, timestamp);
