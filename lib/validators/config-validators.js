...
/** @const {number} Bcrypt hash length (always 60 characters) */
 ...
 * SEC-002: Validates that the admin API key hash is a valid bcrypt hash
 * Bcrypt hashes have format: $2a$10$... or $2b$10$... (60 characters total)
...
function isValidBcryptHash(hash) {
    ...
    if (hash.length !== BCRYPT_HASH_LENGTH) {
        return false;
    }

    // Bcrypt hash format: $2a$, $2b$, or $2y$ followed by cost and hash
...
function validatePathConfig(config, errors) {
    ...
    // Special case: '*' means PUBLIC_MODE - allow any valid absolute path
    const isPublicMode = config.allowedPaths.length === 1 && config.allowedPaths[0] === '*';

    // Validate that all allowed paths are absolute (unless PUBLIC_MODE)
    if (!isPublicMode) {
 ...
 * SEC-002: Updated to validate bcrypt hash instead of plain text API key
 * @param {Object} config - Configuration object
 * @param {Array<string>} errors - Array to collect validation errors
 * @param {Object} logger - Logger instance for warnings
 */
function validateAdminAuth(config, errors, _logger) {
    // Validate admin IPs
    ...
    // SEC-002: Validate admin API key hash (optional, must be valid bcrypt hash if provided)
    if (config.adminApiKeyHash !== null) {
        if (typeof config.adminApiKeyHash !== 'string' || config.adminApiKeyHash.trim().length === 0) {
            errors.push('ADMIN_API_KEY_HASH must be a non-empty string if provided');
        } else if (!isValidBcryptHash(config.adminApiKeyHash)) {
            errors.push(
                'ADMIN_API_KEY_HASH must be a valid bcrypt hash (60 characters, format: $2a$10$... or $2b$10$...). ' +
                'Generate with: node -e "console.log(require(\'bcrypt\').hashSync(\'your-key\', 10))"'
            );
        }
    }
}
