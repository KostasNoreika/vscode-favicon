 ...
 * - Admin Authentication (SEC-001, SEC-002 fix)
 */

const _crypto = require('crypto'); // Reserved for future CSRF token generation
...
function createCSRFProtection() {
    // Endpoints exempt from CSRF check (protected by other mechanisms)
    const csrfExemptPaths = [
        '/api/paste-image', // Protected by: rate limit, path validation, file validation, CORS
    ];
 ...
 *
 * SECURITY FIX SEC-002: Bcrypt hash-based API key authentication
 * Prevents plain text API key exposure in memory via memory dumps
 *
 * @returns {Function} Admin authentication middleware (async)
 */
function createAdminAuth() {
    ...
    // FIX SEC-002: Bcrypt hash-based authentication (prevents memory exposure)
    return async (req, res, next) => {
        ...
        if (clientIP === 'unknown') {
            ...
            return sendError(res, 403, ErrorCodes.ACCESS_DENIED, 'Forbidden');
        }

        // SECURITY FIX SEC-002: API key authentication with bcrypt hash comparison
        // If adminApiKeyHash is configured, require BOTH IP whitelist AND API key
        if (config.adminApiKeyHash) {
            ...
            if (!providedKey) {
                req.log.warn(
                    ...
                    'Admin authentication failed: missing API key'
                );
                return sendError(res, 403, ErrorCodes.ACCESS_DENIED, 'Forbidden');
            }

            try {
                // SECURITY: Use bcrypt.compare() to validate against hash
                // This prevents timing attacks and protects the key from memory dumps
                // bcrypt comparison is intentionally slow to resist brute force
                const isValidKey = await bcrypt.compare(providedKey, config.adminApiKeyHash);
                ...
                // Bcrypt comparison error (should not happen with valid hash)
                req.log.error(
                    { err: error, ip: clientIP },
                    'Admin authentication error: bcrypt comparison failed'
                );
                return sendError(res, 500, ErrorCodes.INTERNAL_ERROR, 'Authentication error');
            }
        }

        // Check if client IP is in the allowed list
        if (!allowedIPs.includes(clientIP)) {
            req.log.warn({ ip: clientIP, allowedIPs }, 'Unauthorized admin access attempt');
            return sendError(res, 403, ErrorCodes.ACCESS_DENIED, 'Forbidden');
        }
