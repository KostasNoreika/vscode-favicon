...
const config = {
    ...
    // SECURITY SEC-002: API key hash for admin endpoints (optional, takes precedence over IP whitelist)
    // If not set, only IP-based authentication is used
    // Must be bcrypt hash of the actual API key (not plain text)
    // Generate hash with: node -e "console.log(require('bcrypt').hashSync('your-key', 10))"
    adminApiKeyHash: process.env.ADMIN_API_KEY_HASH || null,
    ...
    // Upload Storage Configuration (CENT-001)
    uploadTtlDays: parseInt(process.env.UPLOAD_TTL_DAYS || '7', 10),
    uploadMaxFilesPerInstallation: parseInt(process.env.UPLOAD_MAX_FILES || '100', 10),
    uploadMaxSizePerInstallation: parseInt(process.env.UPLOAD_MAX_SIZE || '104857600', 10), // 100MB
    uploadCleanupIntervalMs: parseInt(process.env.UPLOAD_CLEANUP_INTERVAL_MS || '3600000', 10), // 1 hour
    publicUrl: process.env.PUBLIC_URL || 'https://favicon-api.noreika.lt',

    // Metadata Validation Configuration
    // Controls limits for notification metadata to prevent storage bloat and DoS
    maxMetadataSizeBytes: parseInt(process.env.MAX_METADATA_SIZE_BYTES || '5120', 10), // 5KB default
    maxMetadataNestingDepth: parseInt(process.env.MAX_METADATA_NESTING_DEPTH || '3', 10),
    allowedMetadataKeys: (
        process.env.ALLOWED_METADATA_KEYS ||
        'duration,user,context,tags,priority'
    )
        .split(',')
        .map((k) => k.trim()),
        ...
        earlyLogger.info(
            {
                ...
                adminAuth: {
                    ips: config.adminIPs,
                    apiKeyHashConfigured: config.adminApiKeyHash !== null,
                },
