...
class SSEConnectionManager {
    ...
    /**
     * Decrements connection counters for rollback scenarios
     * Pure function that handles both global and per-IP counter decrements
     *
     * @param {string} ip - Client IP address
     * @param {Object} options - Rollback options
     * @param {boolean} options.global - Whether to decrement global counter
     * @param {boolean} options.perIP - Whether to decrement per-IP counter
     * @param {number} options.previousPerIPCount - Previous per-IP count for rollback
     * @private
     */
    decrementCounters(ip, options = {}) {
        if (options.global) {
            this.globalSSEConnections--;
        }

        if (options.perIP && options.previousPerIPCount !== undefined) {
            this.sseConnections.set(ip, options.previousPerIPCount);
        }
    }
     ...
     * QUA-012: Standardized error response format
     * REF-012: Reduced nesting with guard clauses and extracted rollback logic
     *
    ...
    validateConnectionLimits(req, ip) {
        ...
        // Guard: Check global limit
        if (this.globalSSEConnections > this.globalLimit) {
            this.decrementCounters(ip, { global: true });
            req.log.warn(
        ...
        // Guard: Check per-IP limit
        if (currentConnections + 1 > this.maxConnectionsPerIP) {
            this.decrementCounters(ip, {
                global: true,
                perIP: true,
                previousPerIPCount: currentConnections,
            });
            req.log.warn(
